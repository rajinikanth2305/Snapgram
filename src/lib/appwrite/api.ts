...
export async function saveUserToDB(user: { accountId: string, name: string, email: string, imageUrl: URL, username?: string }) {
  try {
    const newUser = databases.createDocument(appwriteConfig.databaseId, appwriteConfig.userCollectionId, ID.unique(), user)
    return newUser;
  }
  catch (error) {
    console.log(error)
...
export async function getCurrentUser() {
  try {
    ...
    console.log(currentUser, "printing here documents")
...
export async function createPost(post: INewPost) {
  try {
    ...
    const uploadFile = await uploadedFile(post.file[0])
    console.log(uploadFile, "checking upload file")
    if (!uploadFile) throw Error;
    //Get file URL
    const fileUrl = getFilePreview(uploadFile.$id)
    console.log(fileUrl, "checking file url here")
    if (!fileUrl) {
      deleteFile(uploadFile.$id)
      throw Error;
    }
    //save post to continue;
    //convert tags to array
    const tags = post.tags?.replace(/ /g, '').split(",") || [];
    //save post to databse
...
export async function getRecentPosts({pageParam}:{pageParam:number}) {
  ...
  try{
    ...
    console.log(posts, "checkign posst to see relation between saves and post")
    return posts;
...
export async function savePost(postId: string, userId: string) {
  try {
    ...
    if (!updatedPost) throw Error;
    console.log(updatedPost, "printign updated post")
    return updatedPost;
  }
  catch (error) {
    console.log(error)
